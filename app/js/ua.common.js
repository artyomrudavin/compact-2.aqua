...
(function() {
	...
	var tenTank = {
		...
		cleanSpirt: '1 л',
		inside: '2.5 л',
		insideButle: '5 пляшок по 0.5',
		oldPrice: '5 590₴',
		newPrice: '4 990₴',
		tankPrice: '2 700₴',
		appName: 'AquaGradus Компакт 14 л',
		tankName: 'Випарна ємність 14 л'
	};

	var twentyTank = {
		urlA: '../img/20/20-ua.jpg',
		urlP: '../img/20/20P-ua.jpg',
		...
		tankName: 'Випарна ємність 25 л'
	};

	var thirtyTank = {
		urlA: '../img/30/30-ua.jpg',
		urlP: '../img/30/30P-ua.jpg',
		...
		tankName: 'Випарна ємність 35 л'
	};

	var fiftyTank = {
		urlA: '../img/30/30-ua.jpg',
		urlP: '../img/50/50P-ua.jpg',
...
(function() {
	...
	$(litreVl).on('click', function (e) {
		...
		if ( bakVal.hasClass('active') ) {
			...
			if ( dataTk == 'twentyTk' ) {
				...
				mtextValue.text('Самогонний апарат Компакт Плюс з баком на 25л');
				hiddenComment.val('Самогонний апарат Компакт Плюс з баком на 25л');
				...
				mtextValue.text('Самогонний апарат Компакт Плюс з баком на 35л');
				hiddenComment.val('Самогонний апарат Компакт Плюс з баком на 35л');
				...
				mtextValue.text('Самогонний апарат Компакт Плюс з баком на 50л');
				hiddenComment.val('Самогонний апарат Компакт Плюс з баком на 50л');
			};
			...
			if ( dataTk == 'tenTk' ) {
				...
				mtextValue.text('Самогонний апарат з баком на 14л');
				hiddenComment.val('Самогонний апарат з баком на 14л');
			} else if ( dataTk == 'twentyTk' ) {
				formOld.text('6 090₴');
	...
	$(colVal).on('click', function(e) {
		...
		if ( colValPC.hasClass('active') ) {
			...
			mtextValue.text('Колона Aquagradus Компакт Плюс');
			hiddenComment.val('Колона Aquagradus Компакт Плюс');
		} else {
			...
			mtextValue.text('Колона Aquagradus Компакт');
			hiddenComment.val('Колона Aquagradus Компакт');
		};


		if ( colVal.hasClass('active') ) {
			colVal.text('Апарат в зборі');
			...
			buyBtn.text('Купити колону');
			modalBtn.text('Купити колону');
		} else {
			...
			mtextValue.text('Самогонний апарат Компакт з баком на 14л');
			hiddenComment.val('Самогонний апарат Компакт з баком на 14л');
			$(litreVl).removeClass('active');
	...
	$(bakVal).on('click', function(e) {
		...
		if ( bakVal.hasClass('active') ) {
			...
			buyBtn.text('Купитb Бак окремо');
			modalBtn.text('Купитb Бак окремо');
		} else {
			bakVal.text('Бак окремо');
			$('#litr3').hide();
			formValue.text('4 990₴');
	...
	$(colValPC).on('click', function(e) {
		...
		if ( colVal.hasClass('active') ) {
			...
			if ( colValPC.hasClass('active') ) {
				...
				formValue.text('2 900₴');
				colValPC.text('колона Компакт Плюс');
				imgConstr.css({
					background: 'url(/img/constructor/pro_big.jpg)',
					backgroundRepeat: 'no-repeat',
					backgroundSize: 'contain',
			...
			mtextValue.text('Самогонний апарат Компакт Плюс з баком 25л');
			hiddenComment.val('Самогонний апарат Компакт Плюс з баком 25л');
			if ( colValPC.hasClass('active') ) {
				colValPC.text('Колона Компакт');
				bakVal.text('Бак окремо');
				$(arrCompl).removeClass('hide');
...
$(function() {
	...
	hiddenComAdd = $('#additionally-modal [name="DATA[COMMENTS]"]');
		...
		if ( $(this).attr('id') == 'carga' ) {
			...
			hiddenComAdd.val('Інноваційний діоптр 4 в 1');
			// console.log('dioptr');
		} else if ( $(this).attr('id') == 'popugai' ) {
			addPrice.text('900₴');
			addTextm.text('Безперервний контроль міцності (Папуга)');
			hiddenComAdd.val('Безперервний контроль міцності (Папуга)');
			// console.log('popugai');
		} else if ( $(this).attr('id') == 'rashiga' ) {
			addPrice.text('850₴');
			addTextm.text('Кільця Рашига 1 кг');
			hiddenComAdd.val('Кільця Рашига 1 кг');
			// console.log('rashiga');
		} else if ( $(this).attr('id') == 'areom' ) {
			addPrice.text('550₴');
			addTextm.text('Набір професійних ареометрів і мірна колба');
			hiddenComAdd.val('Набір професійних ареометрів і мірна колба');
			// console.log('areom');
		} else if ( $(this).attr('id') == 'komplOhl' ) {
			addPrice.text('750₴');
			addTextm.text('Комплект охложденія ДеЛюкс 6 мм');
			hiddenComAdd.val('Комплект охложденія ДеЛюкс 6 мм');
			// console.log('drogi');
